    ...
    <>
      ...
      <main className="p-6 md:p-8 max-w-6xl w-full mx-auto grid lg:grid-cols-3 gap-5">
        <Card className="p-5">
          ...
          <div className="text-4xl font-bold text-primary mt-1">{ats.score}<span className="text-base text-muted-foreground">/100</span></div>
          <Progress value={ats.score} className="mt-3" />
        </Card>
        <Card className="p-5">
          <div className="text-sm text-muted-foreground">Resume Quality</div>
          <div className="text-4xl font-bold mt-1">{ats.quality}<span className="text-base text-muted-foreground">/100</span></div>
          <Progress value={ats.quality} className="mt-3" />
        </Card>
        <Card className="p-5">
          <div className="text-sm text-muted-foreground">Job Match</div>
          <div className="text-4xl font-bold mt-1">{match.pct}<span className="text-base text-muted-foreground">%</span></div>
            ...
            {ats.breakdown.map((b) => (
              <div key={b.label} className="flex items-center justify-between text-sm border-b last:border-0 pb-2">
                <div className="flex items-center gap-2">
                  {b.ok ? <Check className="h-4 w-4 text-emerald-600"/> : <X className="h-4 w-4 text-destructive"/>}
                  <span>{b.label}</span>
                </div>
                <span className="font-mono text-xs text-muted-foreground">{b.points}/{b.max}</span>
              </div>
            ))}
          </div>
        </Card>
